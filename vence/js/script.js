...
new Swiper('.slider-art', {
    pagination: {
        el: '.slider-art__pagination',
        dymamicBullets: true,
...
const latestPoints = document.querySelectorAll('.menu-latest__point');

latestPoints.forEach(function(latestPoint) {
    latestPoint.addEventListener('click', function(e) {
        latestPoints.forEach(function(point) {
            point.classList.remove('menu-latest__point_active');
        });
        latestPoint.classList.add('menu-latest__point_active'); 
    });
});

const menuLinks = document.querySelectorAll('.menu-latest__link');

menuLinks.forEach(function(link) {
  link.addEventListener('click', function(e) {
    e.preventDefault(); // Відключення оновлення сторінки
    // Додаткові дії при кліку на посилання
  });
});
